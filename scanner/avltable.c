...
{
    if (p != NULL) {
        NT_ASSERT (!FlagOn((ULONG)p, 3) && FlagOn((ULONG)p, 0x80000000));
    }
    return p;
}
#else
...
{
    if (AvlIsGenericTableEmptyAvl(Table)) {
...

{
    ...
    for (i=2; i < 33; i++) {
        NT_ASSERT (WorstCaseFill[i] == (1 + WorstCaseFill[i-1] + WorstCaseFill[i-2]));
    }
#endif
    ...
    DkpZeroMemory( Table, sizeof(RTL_AVL_TABLE) );
...

{
    ...
    return AvlInsertElementGenericTableFullAvl(
                Table,
                Buffer,
                BufferSize,
                NewElement,
...
Routine Description:
    ...
    previous AvlLookupElementGenericTableFullAvl.
...
Arguments:
   ...
   NodeOrParent - Result of prior AvlLookupElementGenericTableFullAvl.

   SearchResult - Result of prior AvlLookupElementGenericTableFullAvl.
...

{
    ...
    if (SearchResult != TableFoundNode) {
        ...
        DkpZeroMemory( NodeToReturn, sizeof(RTL_BALANCED_LINKS) );
        ...
        DkpCopyMemory( &NodeToReturn->UserData, Buffer, BufferSize );
...

{
    ...
    if (Lookup != TableFoundNode) {
        ...
        //
        //  Make AvlEnumerateGenericTableAvl safe by replacing the RestartKey
        ...
        //
        //  Make AvlEnumerateGenericTableLikeADirectory safe by incrementing the
        //  DeleteCount.
        //
...

{
    ...
    return AvlLookupElementGenericTableFullAvl(
                Table,
                Buffer,
                &NodeOrParent,
                &Lookup
...

{
    ...
    return AvlEnumerateGenericTableWithoutSplayingAvl( Table, &Table->RestartKey );
}
...
Routine Description:
    ...
    I = (AvlNumberGenericTableElements2(Table)-1) will return the last/highest
    ...
    COMMENTS FOR AvlEnumerateGenericTableLikeADirectory.
...
Routine Description:
    ...
    To summarize the four(!) enumeration routines and when to use them:
      ...
      - For the simplest way for a single thread to enumerate the entire table
        ...
        AvlEnumerateGenericTableAvl.  This routine is not reentrant, and thus
      ...
      - For the simplest way for multiple threads to enumerate the entire table
        in collation order and in parallel, use AvlEnumerateGenericTableWithoutSplayingAvl.
      ...
      - For the simplest way for multiple threads to enumerate the entire table
        ...
        use AvlGetElementGenericTableAvl.  This routine requires only shared access
      ...
      - To enumerate the table in multiple threads in collation order, safely
        ...
        calls, use AvlEnumerateGenericTableLikeADirectory.  This is the only routine
...
Routine Description:
    ...
    This routine finds the left-most element in the tree matching the data in Buffer.
    If, for example, the tree contains filenames there may exist several that differ
    only in case. A case-blind searcher can use this routine to position himself in
    the tree at the first match, and use an enumeration routine (such as
    AvlEnumerateGenericTableWithoutSplayingAvl) to return each subsequent match.

Arguments:

    Table - Pointer to the generic table.

    Buffer - Used for the comparison.

    RestartKey - Returns a search context for use with an enumeration routine.

Return Value:

    PVOID - Returns a pointer to the user data, or NULL if no match was found.

--*/

{
    PRTL_BALANCED_LINKS ThisNode, PrevNode;
    TABLE_SEARCH_RESULT SearchResult;
    RTL_GENERIC_COMPARE_RESULTS CompareResult = GenericEqual;

    //
    //  In case we find nothing, don't return a RestartKey.
    //

    *RestartKey = NULL;

    SearchResult = FindNodeOrParent( Table,
                                     Buffer,
                                     &ThisNode );

    //
    //  If we didn't find any match, get out now.
    //
    ...
    //
    //  Now look at predecessors until we run out of matches.
    //

    PrevNode = ThisNode;

    while (CompareResult == GenericEqual) {

        ThisNode = PrevNode;

        PrevNode = RealPredecessor( ThisNode );

        if (PrevNode == NULL) {
            break;
        }

        CompareResult = Table->CompareRoutine( Table,
                                               Buffer,
                                               &((PTABLE_ENTRY_HEADER) PrevNode)->UserData );
    }

    //
    //  Set the restart key to the earliest matching node.
    //
    ...
    *RestartKey = ThisNode;
    ...
    return &((PTABLE_ENTRY_HEADER) ThisNode)->UserData;
}


static
NTSTATUS
_EnumerateSubtreeWithCallback(
    __in PRTL_BALANCED_LINKS Subtree,
...
Routine Description:
    ...
    The function _EnumerateSubtreeWithCallback takes as input a subtree in
    a generic AVL table, a callback, and a parameter to be passed to that
    callback.  It calls that callback for each node in the subtree, in
    order, stopping when the callback returns something other than
    STATUS_SUCCESS.  It returns the last returned value from the callback.

Arguments:

    Subtree - Pointer to the links field of the root of the subtree to be
              enumerated.

    Callback - User routine to be called for each node in the table.

    CallbackParameters - Parameter to be passed to the callback each time
                         it is invoked.

Return Value:

    The last value returned from the callback.  If the subtree is empty and
    thus the callback is never called, the return value is STATUS_SUCCESS.
...
--*/
{
    ...
    Status = _EnumerateSubtreeWithCallback(Subtree->LeftChild, Callback, CallbackParameters);
    if (Status != STATUS_SUCCESS) {
        return Status;
    }
    
    Status = (*Callback)(CallbackParameters, &((PTABLE_ENTRY_HEADER) Subtree)->UserData);
    if (Status != STATUS_SUCCESS) {
        return Status;
    }

    Status = _EnumerateSubtreeWithCallback(Subtree->RightChild, Callback, CallbackParameters);
    return Status;
}
...
Routine Description:
    ...
    The function AvlEnumerateGenericTableWithCallback takes as input a
    generic AVL table, a callback, and a parameter to be passed to that
    callback.  It calls that callback for each node in the table, in order,
    stopping when the callback returns something other than STATUS_SUCCESS.
    It returns the last returned value from the callback.

Arguments:

    Table - Pointer to the generic table to be enumerated.
...
Return Value:
    ...
    The last value returned from the callback.  If the tree is empty and
    thus the callback is never called, the return value is STATUS_SUCCESS.

--*/
{
    return _EnumerateSubtreeWithCallback(Table->BalancedRoot.RightChild, Callback, CallbackParameters);
}
